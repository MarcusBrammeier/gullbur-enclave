...
async fn e2e_relay_envelope_to_websocket() {
    ...
    // Auth
    ws_write
        .send(Message::Text(auth_token.into()))
        .await
        .expect("auth");
    ...
    ws_write
        .send(Message::Text(rpc.to_string().into()))
        .await
        .expect("send");
    ...
    let routed = router::route("eth_requestAccounts", json!({"seed_phrase":"test"}))
    ...
    let routed =
        router::route("eth_estimateGas", json!({"network":"ethereum"})).expect("test invariant");
    ...
    let routed =
        router::route("eth_sendTransaction", json!({"from":"0x123"})).expect("test invariant");
    assert!(routed.requires_approval);
