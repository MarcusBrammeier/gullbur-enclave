...
async fn rpc_call<W, R>(write: &mut W, read: &mut R, method: &str, params: serde_json::Value) -> Result<serde_json::Value, String>
where
    ...
    R: StreamExt<Item = Result<Message, tokio_tungstenite::tungstenite::Error>> + Unpin,
{
    ...
    let p: serde_json::Value = serde_json::from_str(msg.to_text().map_err(|_| "bin")?).expect("test invariant");
...
async fn e2e_disconnect_reconnect() {
    ...
    w1.send(Message::Text(r#"{"type":"hello"}"#.into())).await.expect("test invariant");
    let msg = tokio::time::timeout(Duration::from_secs(5), r1.next()).await.expect("test invariant").expect("test invariant").expect("test invariant");
    ...
    let r = rpc_call(&mut w1, &mut r1, "vault.generate_mnemonic", serde_json::json!({})).await.expect("test invariant");
    ...
    w2.send(Message::Text(r#"{"type":"hello"}"#.into())).await.expect("test invariant");
    let msg = tokio::time::timeout(Duration::from_secs(5), r2.next()).await.expect("test invariant").expect("test invariant").expect("test invariant");
    let v: serde_json::Value = serde_json::from_str(msg.to_text().expect("test invariant")).expect("test invariant");
    assert_eq!(v["type"], "session_key");
    println!("[dr] Connection 2: new session key ✓");

    // Initialize with the mnemonic from connection 1
    let r = rpc_call(&mut w2, &mut r2, "vault.initialize", serde_json::json!({"seed_phrase": mnemonic, "passphrase": ""})).await.expect("test invariant");
    assert!(r["initialized"].as_bool().expect("test invariant"), "initialize should succeed on fresh connection");
    println!("[dr] Connection 2: initialized vault ✓");

    // Verify status
    let r = rpc_call(&mut w2, &mut r2, "vault.status", serde_json::json!({})).await.expect("test invariant");
    assert!(r["initialized"].as_bool().expect("test invariant"));
    assert!(!r["plugin_ids"].as_array().expect("test invariant").is_empty());
