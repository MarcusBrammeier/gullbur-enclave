...
async fn e2e_plaintext_connect_and_generate_mnemonic() {
    let server = ipc_core::server::IpcServer::with_encryption(TEST_PORT, true)
    ...
    let (ws, _) = connect_async(format!("ws://127.0.0.1:{TEST_PORT}"))
        .await.expect("WebSocket connect");
    let (mut write, mut read) = ws.split();

    // Hello handshake (loopback trust)
    write.send(Message::Text(r#"{"type":"hello"}"#.into())).await.expect("send hello");
    ...
    let session_val: serde_json::Value = serde_json::from_str(&session_text).expect("test invariant");
    ...
    write.send(Message::Text(rpc.into())).await.expect("send rpc");
    ...
    assert!(resp_val.get("__encrypted__").is_none(),
        "plain request must get plain response: {resp_text}");
    assert!(resp_val.get("result").is_some(),
