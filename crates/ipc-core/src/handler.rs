...
use ipc_protocol::{JsonRpcError, JsonRpcRequest, JsonRpcResponse, RpcError};
...
impl MessageHandler {
    ...
    pub async fn dispatch(&self, request: JsonRpcRequest) -> DispatchResult {
        ...
        match self.handlers.get(&request.method) {
            ...
            None => DispatchResult::Error(JsonRpcError::new(RpcError::method_not_found(), id)),
        }
    }
}
...
mod tests {
    ...
    async fn test_handler_register_and_dispatch() {
        ...
        let req = JsonRpcRequest::new("echo", Some(serde_json::json!({"hello": "world"})), 42);
        match handler.dispatch(req).await {
