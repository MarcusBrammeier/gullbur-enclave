...
async fn get_balance_routes() {
    ...
    let r = call(PORT, &token, "vault.get_balance", serde_json::json!({
        "network": "ethereum",
    ...
    if let Some(error) = r.get("error") {
        ...
        assert!(result.get("unit").is_some() || result.get("balance").is_some(),
...
async fn estimate_fee_routes() {
    ...
    let r = call(PORT + 1, &token, "vault.estimate_fee", serde_json::json!({
        "network": "bitcoin"
    })).await;
...
async fn transaction_history_routes() {
    ...
    let r = call(PORT + 2, &token, "vault.get_transaction_history", serde_json::json!({
        "network": "ethereum",
        "address": "0x1234567890123456789012345678901234567890",
        "limit": 5
    })).await;
    ...
    if let Some(error) = r.get("error") {
        let code = error["code"].as_i64().unwrap_or(0);
        assert_ne!(code, -32601, "get_transaction_history must be a registered handler");
    }
}
...
async fn list_networks() {
    ...
    let r = call(PORT + 3, &token, "vault.list_networks", serde_json::json!({})).await;
    ...
    assert!(networks.len() >= 5, "expected >=5 networks, got {}", networks.len());
}
...
async fn list_plugins() {
    ...
    let ids: Vec<&str> = plugin_ids.iter().map(|v| v.as_str().expect("test invariant")).collect();
