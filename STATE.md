...
| GitHub URLs point at `MarcusBrammeier/gullbur-enclave` | Repo exists under this path (private). Consistent across all crates, README, update-checker, crash-report link. | ✅ Confirmed — no placeholder; the repo is live at this location |
| **XMR wallet-rpc balance/history needs a P2P daemon** | The daemon JSON-RPC path (broadcast/fee/decoys) is fixed with configurable failover. Balance/history via `monero-wallet-rpc` needs a P2P daemon; no reliable *public* stagenet P2P node exists, but a **local `monerod --stagenet`** works. Now proven: live test (`xmr_wallet_rpc_live`) connects wallet-rpc to a local synced stagenet node (127.0.0.1:38081, synchronized) and answers get_address/get_balance. | ✅ P2P requirement met locally; a funded stagenet wallet is still needed for a live sign→broadcast |
| **Live broadcast confirmed — BTC testnet4 & EVM Sepolia** | Public-chain broadcast proven end-to-end. BTC testnet4 (`tb1qxfw0jn…`) shows 3 confirmed txs incl. a 1000-sat-fee spend confirmed in block 148333 (txid `e64e895c…`); EVM Sepolia broadcast accepted (creds `0x6b9b…`, VERIFIED). | ✅ Broadcast path live on BTC & EVM |
| **LTC public testnet relay awaits supplied funding** | Sign+broadcast verified **on local regtest** (txid `56992f8c…`, 1 confirm). Relay path is the same Esplora broadcast BTC testnet4 proved end-to-end. Pending only **testnet LTC at our address** — no working faucet (cypherfaucet relay-gapped, bitaps CF-gated, others dead) and this box lacks disk for a local testnet3 sync. Once testnet3/4 LTC lands + confirms, run `live_broadcast` and it relays/confirms on the public chain. | Waiting on supplied/community testnet LTC funding; non-blocking |
